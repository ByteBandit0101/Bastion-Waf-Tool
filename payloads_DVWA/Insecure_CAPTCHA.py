...
url_alvo = f"{{url_base}}DVWA/vulnerabilities/insecure_captcha"
...
for nome_payload, payload in payloads.items():
    ...
    if resposta_teste.status_code == 200:  # Modificar a lógica de verificação conforme necessário
        print(f"Teste #{{total_testes}} PASSOU: Vulnerabilidade 'Insecure CAPTCHA' detectada com payload '{{nome_payload}}'!")
        testes_passaram += 1
    else:
