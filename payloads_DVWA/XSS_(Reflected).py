...
url_alvo = f"{url_base}DVWA/vulnerabilities/xss_r"  # URL para XSS Reflected
...
for payload in payloads:
    ...
    resposta_teste = requests.get(url_alvo, headers=headers, params=params)

    if resposta_teste.status_code == 200 and payload in resposta_teste.text:
        print(f"Teste #{total_testes} PASSOU: Vulnerabilidade 'XSS Reflected' detectada com payload '{payload}'!")
        testes_passaram += 1
    else:
